...
But, once you've learned to hold the TypeScript violin just right, the typechecking in your repo will be fast and you can iterate safely.

Let's build a package for a TypeScript/React UI library to find out how this works.
...
We'll add a `package.json` that exports a `base.json` file:
...
## Making the TypeScript compiler fast in a monorepo

There are two strategies that we will use to make `tsc` fast:
...
2.  Caching workspace tasks with our package manager so that we can ensure we never do the same work twice.

Putting these two techniques together, we'll be caching the typecheck task for entire workspaces so we don't do the same work twice **and** be as fast as possible when we miss the cache to check types.
...
1.  `typescript`
2.  `@acme/tsconfig`, the package with our base TSConfig.
...
```json
{
  ...
  "dependencies": {
    "@acme/tsconfig": "workspace:*" // Use "*" for npm or yarn
  },
  "devDependencies": {
    "typescript": "5.1.6"
  }
...
Next, we'll create a `tsconfig.json` in our workspace.

<FileTree>
  <FileTree.Folder name="packages" open>
    <FileTree.Folder name="ui" open>
      <FileTree.File name="package.json" />
...
In the `tsconfig.json`, we'll extend from our base configuration:

```json
{
  "$schema": "https://json.schemastore.org/tsconfig",
  "extends": "@acme/tsconfig/base.json", // Using our base config!
  "compilerOptions": {
...
Most of this file probably looks familiar - but what's this `tsBuildInfoFile` property?
...
If all goes according to plan, you'll see your task ran by Turborepo in your terminal and a `tsbuildinfo.json` file in the `node_modules/.cache` directory of your workspace. You can repeat this process in as many workspaces as you'd like.

### Building our Turborepo pipeline
...
<Callout type="goodToKnow">
  <li>
    ...
    still find it advantageous to extend from a common base configuration.
  </li>
  <li>
