...
ESLint is the most common linter used in the JavaScript world today. Setting it up in a monorepo can be difficult and pieceing together the various configurations can be frustrating if you don't know what you need to do.
...
Not all of our apps should use the same linting configuration. As an example, default exports tend to be inadvisable for JavaScript modules but some frameworks require default exports to work properly (e.g. A Next.js `page.js` file needs a default export).

To create some presets for our workspaces, we'll set up a workspace with our conigurations in `tooling/eslint-config`.
...
Now, we'll create a `node.js` file in our workspace:
...
And, a configuration to use in our Next.js apps:
...
Now, we'll want to use these presets. To do so, we'll do two things:

1. Create an `.eslintrc.js` file in the workspace.

```js filename="packages/ui" lang="js"
...
```json filename="packages/ui" lang="js"
{
  "name": "@repo/ui",
  "version": "0.0.0",
  "private": true,
...
```js filename="packages/ui/.eslintrc" lang="js"
module.exports = {
...
In the root of our monorepo, we will create this script:
