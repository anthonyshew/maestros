...
type AllNodes = typeof rawCiNodes[number]['data']['label'] |
...
export const handleIntergroupEdge = ({ sourceGroup, targetGroup, edge }: {
  ...
  if (sourceGroup === targetGroup) {
    throw new Error(`handleIntegroupEdge: sourceGroup and targetGroup cannot match.`)
  }
  ...
  return ({
    ...edge,
    id: `${getParentNodeId(sourceGroup)}-${getParentNodeId(targetGroup)}-${targetGroup}-${edge.source}-${edge.target}`,
