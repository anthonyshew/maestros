...
### Ignoring workspaces for your root task

We don't want our root task to be responsible for formatting the entire repository. We can take advantage of more granular caching if our root Prettier script is only responsible for root files (not workspace files).

To ignore workspaces for your root task, add a `.prettierignore` to your root:

```js filename="./.prettierignore" note="This will not impact the ignore behavior within your workspaces. See next section."
